...
/* ══════════════════════════════════════════════════════════════════
   SUPABASE COURSE FEES — Load & Save
   Table: course_fees (course_id, course_name, subtype, monthly_fee, admission_fee)
   ══════════════════════════════════════════════════════════════════ */

/* Load all fees from Supabase and patch into GNSI_COURSES + FS_FEE_CONFIG */
function gnsiLoadFeesFromSupabase(callback) {
  var url = GNSI_SUPABASE_URL + '/rest/v1/course_fees?select=*&order=course_id,subtype';
  fetch(url, {
    headers: {
      'apikey': GNSI_SUPABASE_KEY,
      'Authorization': 'Bearer ' + GNSI_SUPABASE_KEY,
  ...
  .then(function(rows) {
    ...
    /* Patch GNSI_COURSES live */
    if (typeof GNSI_COURSES !== 'undefined') {
      GNSI_COURSES.forEach(function(gc) {
        gc.subTypes.forEach(function(st) {
          var subtype = st.id.replace(gc.id + '_', '');
          var row = rows.find(function(r) { return r.course_id === gc.id && r.subtype === subtype; });
          if (row) st.monthlyFee = row.monthly_fee || 0;
        });
      });
    }
    /* Patch FS_FEE_CONFIG if available */
    if (typeof FS_FEE_CONFIG !== 'undefined' && FS_FEE_CONFIG.courses) {
      FS_FEE_CONFIG.courses.forEach(function(c) {
        ...
        ['boarder','dayscholar','dayboarder'].forEach(function(st) {
          var row = rows.find(function(r) { return r.course_id === c.id && r.subtype === st; });
          if (row) {
            c.monthlyFees[st] = row.monthly_fee || 0;
            if (st === 'boarder') c.admFee = row.admission_fee || 0;
          }
        });
      });
    }
    /* Also patch loadFeeConf store */
    try {
      if (typeof loadFeeConf === 'function' && typeof saveFeeConf === 'function') {
        ...
        rows.forEach(function(row) {
          (_mc.monthlyFees || []).forEach(function(mf) {
            ...
            if (cn.indexOf(row.course_id) !== -1) {
              if (row.subtype === 'boarder')    mf.amount       = row.monthly_fee || 0;
              if (row.subtype === 'dayscholar') mf.hostelAmount = row.monthly_fee || 0;
            }
          });
        });
        saveFeeConf(_mc);
    ...
    if (typeof callback === 'function') callback(true, rows);
  })
  .catch(function(err) {
    console.warn('GNSI: Supabase fee load failed', err);
    if (typeof callback === 'function') callback(false, err);
  });
}

/* Save one course's fees to Supabase (upsert by course_id + subtype) */
function gnsiSaveFeeToSupabase(courseId, courseName, subtype, monthlyFee, admissionFee, callback) {
  var url = GNSI_SUPABASE_URL + '/rest/v1/course_fees';
  fetch(url, {
    method: 'POST',
    headers: {
      'apikey': GNSI_SUPABASE_KEY,
    ...
    body: JSON.stringify({
      ...
      admission_fee: admissionFee || 0,
      updated_at:    new Date().toISOString()
    })
  })
  .then(function(res) {
    if (typeof callback === 'function') callback(res.ok);
  })
  .catch(function(err) {
    console.warn('GNSI: Supabase fee save failed', err);
    if (typeof callback === 'function') callback(false);
  });
}

/* Save ALL courses fees to Supabase at once */
function gnsiSaveAllFeesToSupabase(feesArray, callback) {
  var url = GNSI_SUPABASE_URL + '/rest/v1/course_fees';
  fetch(url, {
    method: 'POST',
    headers: {
  ...
  .catch(function(err) {
    console.warn('GNSI: Supabase bulk fee save failed', err);
    if (typeof callback === 'function') callback(false);
  });
}

/* Render the Admin Fee Manager UI — call inside any page render */
function renderAdminFeeManager() {
  var isAdm = currentUser && (currentUser.role === 'admin' || currentUser.role === 'manager');
  if (!isAdm) return '<div style="padding:20px;color:var(--muted);text-align:center">🔒 Admin access required to manage fees.</div>';

  var courses = (typeof GNSI_COURSES !== 'undefined') ? GNSI_COURSES : [];
  var subtypes = [
    { key: 'boarder',    label: 'Boarder',    icon: '🏠', color: '#1a6b55' },
    { key: 'dayscholar', label: 'Day Scholar', icon: '🚌', color: '#1433a8' },
    { key: 'dayboarder', label: 'Day Boarder', icon: '🔄', color: '#7c3aed' }
  ];

  var rows = courses.map(function(c, ci) {
    var stCells = subtypes.map(function(st) {
      var currentFee = 0;
      var stObj = c.subTypes ? c.subTypes.find(function(s) { return s.id === c.id + '_' + st.key; }) : null;
      if (stObj) currentFee = stObj.monthlyFee || 0;
      return '<td style="padding:8px">'
        + '<div style="font-size:10px;color:' + st.color + ';font-weight:700;margin-bottom:3px">' + st.icon + ' ' + st.label + '</div>'
        + '<input id="adm-fee-' + ci + '-' + st.key + '" type="number" min="0" value="' + currentFee + '" '
        + 'style="width:100%;padding:7px 10px;border:1.5px solid var(--border);border-radius:8px;font-size:13px;font-weight:600;text-align:right" '
        + 'placeholder="0" oninput="gnsiAdminFeePreview(' + ci + ',\'' + st.key + '\',this.value,\'' + st.color + '\')">'
        + '<div id="adm-prev-' + ci + '-' + st.key + '" style="margin-top:3px;min-height:18px;text-align:right"></div>'
        + '</td>';
    }).join('');

    /* Admission fee — use boarder row's admFee */
    var admFee = 0;
    if (typeof FS_FEE_CONFIG !== 'undefined' && FS_FEE_CONFIG.courses) {
      var fsc = FS_FEE_CONFIG.courses.find(function(x) { return x.id === c.id; });
      if (fsc) admFee = fsc.admFee || 0;
    }

    return '<tr style="border-bottom:1px solid var(--border)">'
      + '<td style="padding:10px 8px;font-weight:800;font-size:14px;white-space:nowrap">' + (c.icon || '📚') + ' ' + c.name + '</td>'
      + stCells
      + '<td style="padding:8px">'
      + '<div style="font-size:10px;color:#d97706;font-weight:700;margin-bottom:3px">🎫 Admission</div>'
      + '<input id="adm-fee-adm-' + ci + '" type="number" min="0" value="' + admFee + '" '
      + 'style="width:100%;padding:7px 10px;border:1.5px solid var(--border);border-radius:8px;font-size:13px;font-weight:600;text-align:right" placeholder="0">'
      + '</td>'
      + '</tr>';
  }).join('');

  return '<div class="card" id="admin-fee-manager-card">'
    + '<div class="card-head">'
    + '<span class="card-title">💰 Fee Manager <span style="font-size:11px;background:#dcfce7;color:#16a34a;border-radius:20px;padding:2px 10px;font-weight:700;margin-left:8px">Supabase Cloud</span></span>'
    + '<div style="display:flex;gap:8px">'
    + '<button onclick="gnsiAdminRefreshFees()" class="btn btn-outline" style="font-size:12px">🔄 Refresh from Cloud</button>'
    + '<button onclick="gnsiAdminSaveAllFees()" class="btn btn-primary" style="font-size:12px">☁️ Save All to Cloud</button>'
    + '</div></div>'
    + '<div style="padding:16px;overflow-x:auto">'
    + '<table style="width:100%;border-collapse:collapse;min-width:600px">'
    + '<thead><tr style="background:var(--surface-alt,#f8fafc)">'
    + '<th style="padding:10px 8px;text-align:left;font-size:12px;color:var(--muted)">COURSE</th>'
    + '<th style="padding:10px 8px;text-align:left;font-size:12px;color:var(--muted)">BOARDER (₹/mo)</th>'
    + '<th style="padding:10px 8px;text-align:left;font-size:12px;color:var(--muted)">DAY SCHOLAR (₹/mo)</th>'
    + '<th style="padding:10px 8px;text-align:left;font-size:12px;color:var(--muted)">DAY BOARDER (₹/mo)</th>'
    + '<th style="padding:10px 8px;text-align:left;font-size:12px;color:var(--muted)">ADMISSION (₹)</th>'
    + '</tr></thead>'
    + '<tbody>' + rows + '</tbody>'
    + '</table>'
    + '</div>'
    + '<div id="adm-fee-status" style="padding:12px 16px;font-size:13px;color:var(--muted);text-align:right">Last synced from Supabase on load.</div>'
    + '</div>';
}

function gnsiAdminFeePreview(ci, st, val, color) {
  var n = parseInt(val, 10);
  var el = document.getElementById('adm-prev-' + ci + '-' + st);
  if (!el) return;
  if (isNaN(n) || n < 0) { el.innerHTML = ''; return; }
  el.innerHTML = '<span style="display:inline-block;padding:1px 8px;border-radius:20px;font-size:11px;font-weight:700;background:' + color + '15;color:' + color + ';border:1px solid ' + color + '33">₹' + n.toLocaleString('en-IN') + '/mo</span>';
}

function gnsiAdminRefreshFees() {
  var statusEl = document.getElementById('adm-fee-status');
  if (statusEl) statusEl.innerHTML = '⏳ Loading from Supabase…';
  gnsiLoadFeesFromSupabase(function(ok, data) {
    if (ok) {
      if (typeof showToast === 'function') showToast('✅ Fees refreshed from Supabase!', '#15803d');
      if (typeof render === 'function') render();
    } else {
      if (typeof showToast === 'function') showToast('❌ Failed to load from Supabase', '#dc2626');
      if (statusEl) statusEl.innerHTML = '❌ Failed to load from cloud.';
    }
  });
}

function gnsiAdminSaveAllFees() {
  var courses = (typeof GNSI_COURSES !== 'undefined') ? GNSI_COURSES : [];
  ...
  courses.forEach(function(c, ci) {
    var admEl = document.getElementById('adm-fee-adm-' + ci);
    var admFee = admEl ? (parseInt(admEl.value, 10) || 0) : 0;

    subtypes.forEach(function(st) {
      var el = document.getElementById('adm-fee-' + ci + '-' + st);
      if (!el) return;
  ...
  if (!allOk) {
    if (typeof showToast === 'function') showToast('⚠️ Some values are invalid', '#dc2626');
    return;
  }

  var statusEl = document.getElementById('adm-fee-status');
  if (statusEl) statusEl.innerHTML = '⏳ Saving to Supabase…';
  if (typeof showToast === 'function') showToast('⏳ Saving fees to cloud…', '#2563eb');

  gnsiSaveAllFeesToSupabase(feesArray, function(ok) {
    if (ok) {
      /* Also sync locally */
      if (typeof fmcSaveFeeStructure === 'function') {
        try { fmcSaveFeeStructure(); } catch(e) {}
      }
      var now = new Date().toLocaleString('en-IN');
      if (statusEl) statusEl.innerHTML = '✅ Saved to Supabase at ' + now;
      if (typeof showToast === 'function') showToast('✅ All fees saved to Supabase cloud!', '#15803d');
      if (typeof render === 'function') render();
    } else {
      if (statusEl) statusEl.innerHTML = '❌ Save failed. Check connection.';
      if (typeof showToast === 'function') showToast('❌ Failed to save to Supabase', '#dc2626');
    }
  });
}
...
async function gnsiFinanceInit() {
  ...
  try {
    ...
    /* ── AUTO-LOAD course fees from Supabase course_fees table ── */
    setTimeout(function(){
      gnsiLoadFeesFromSupabase(function(ok){
...
function renderUnifiedFeeHub() {
  ...
  if (activeTab === 'feesetup')html += '<div>' + (currentUser&&(currentUser.role==='admin'||currentUser.role==='manager') ? renderAdminFeeManager() + '<div style="margin-top:18px"></div>' : '') + _fmcRenderFeeSetup(isAccounts) + _fmcRenderClassBridge(isAccounts) + '</div>';
